...
def strip_markdown(text):
    ...
    # 이미지 ![설명](URL)은 줄째로 제거 — 그 이미지가 곧 이 게시물이라 캡션에
    # 다시 URL을 적을 이유가 없다. 반드시 아래 링크 변환보다 먼저 처리할 것
    # (뒤에 하면 [..](..)만 걷혀 앞의 '!'가 고아로 남는다).
    text = re.sub(r'!\[[^\]]*\]\([^)]*\)', '', text)
    # 볼드 기호 ** 제거
    text = text.replace('**', '')
    ...
    for line in lines:
        if line.strip().startswith('|'):
            ...
            cleaned_lines.append(re.sub(r'^\s*#{1,6}\s+', '', line))
    # 이미지를 걷어낸 자리에 생긴 빈 줄 정리
