...
def draft_zone(adv, r, seq, total, total_zones):
    ...
    # 한계는 밝히되 스스로를 깎지 않는다. 옛 문장의 "여러 힘 가운데 하나일
    # 뿐입니다"는 방금 보여준 분석을 자기 부정으로 만들었고, 이 지표가 무엇을
    # 하는지는 한 마디도 안 했다(2026-08-15 사용자 지적).
    body.append('<p>다만 이건 <b>가격을 예측하는 지표가 아니라 공급을 재는 자</b>입니다. '
                '어느 쪽으로 기울어 있는지는 알려주지만, 언제 움직일지는 정하지 '
                '못합니다. 특히 금리가 크게 흔들리면 이 순환이 한동안 멈춥니다 — '
                '2021년 저금리 때가 그랬습니다.</p>')
