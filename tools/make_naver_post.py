...
def draft_zone(adv, sts, r, seq, total, total_zones):
    ...
    if mine is not None and r.get('um') is not None:
        ...
        body.append('<p>공급이 모자란 곳에서는 <b>전세가 먼저 오르고, 그 전세가 '
                    '매매를 밉니다.</b> 이 관계는 분석한 시도 전부에서 같은 '
                    '방향으로 확인됐습니다. 정하지 못하는 건 <b>시점</b>입니다 — '
                    '금리가 크게 흔들리면 순환이 한동안 멈춥니다.</p>')
    ...
    body.append('<p>다른 지역도 같은 기준으로 판정해 두었습니다. 내 동네가 어느 '
                '칸에 있는지 확인해 보세요.</p>')
    # ── 산식은 맨 뒤에, 세 문장으로. 예전엔 이게 결론 직후에 다섯 문단이었다.
    # 매주 읽는 사람에게는 반복이고, 처음 온 사람에게는 지역 정보를 가로막는다.
    # 자세한 건 리포트가 지고, 블로그는 '이 지역이 어떤가'를 말한다.
    body.append('<h3>숫자는 이렇게 냈습니다</h3>')
    body.append('<p><b>적정물량</b>은 이 지역 가격이 하락에서 상승으로 돌아섰던 '
                '시점의 입주물량을 실측해 잡은 기준선입니다. 인구로 나눈 추정치가 '
                '아닙니다.</p>')
    body.append('<p>앞으로의 공급은 <b>인허가가 아니라 착공</b>으로 셉니다. 인허가는 '
                '삽을 안 뜬 계획이 섞여 실제보다 1.3~1.7배 부풉니다. 착공한 현장이 '
                '입주까지 %d년쯤 걸리므로 그만큼 미래로 밀어 셉니다.</p>' % yrs)
    body.append('<p>앞으로 헐릴 집은 빼지 않았습니다. 재건축 시기를 미리 알 수 없어서인데, '
                '그만큼 <b>실제 부족은 이 숫자보다 큽니다.</b></p>')
