...
def draft_zone(adv, sts, r, seq, total, total_zones):
    ...
    body.append('<p>시도 단위로 보는 건 국토교통부 통계가 시도 경계로 나오기 '
                '때문입니다. 화면의 숫자를 <b>원자료와 그대로 맞춰볼 수 '
                '있습니다.</b></p>')
    # 부정형으로 시작하면 차별점이 변명으로 읽힌다. 우리가 실측했다는 게 요지고,
    # 인구 추정치가 아니라는 건 그 뒤에 붙는 대조다(2026-08-15 사용자 지적).
    body.append('<p><b>적정물량</b>은 실측한 기준선입니다. 과거 이 지역의 가격이 '
                '하락에서 상승으로 방향을 바꾼 시점의 입주물량을 재서 잡았습니다. '
                '그만큼 들어오면 시장이 돌아섰다는 뜻이니까요. 인구로 나눈 '
                '추정치가 아닙니다.</p>')
    ...
    body.append('<p>게다가 이 숫자는 <b>보수적입니다.</b> 앞으로 헐릴 집을 빼지 '
                ...
                '방법이 없어서입니다. 그만큼 <b>실제 부족은 이보다 큽니다.</b> '
                '노후 아파트가 많은 지역일수록 그렇습니다.</p>' % yrs)
    ...
    # ⚠️ "가격을 예측하지 못한다"고 쓰면 안 된다. 우리 리포트가 그 반대를
    # 증명했다 — 전세→매매 고리는 분석한 15개 시도 전부에서 같은 방향,
    # 재현율 100%, 평균 상관 0.71로 6개 고리 중 가장 보편적이다. 그걸 부정하면
    # 우리 데이터를 우리가 깎는다(2026-08-15 사용자 지적, 두 번 고쳤다).
    #
    # 정확한 선은 **방향은 되고 시점·폭은 안 된다**는 것이다. 금리가 그 위를
    # 덮어쓰기 때문이다(2021 저금리·2022 급등 실측).
    # 강한 것을 먼저, 한계는 뒤에. '다만…대신…못합니다'로 감싸면 전 시도에서
    # 확인된 가장 단단한 고리가 약해 보인다(2026-08-15 사용자 지적).
    body.append('<p>공급이 모자란 곳에서는 <b>전세가 먼저 오르고, 그 전세가 매매를 '
                '밉니다.</b> 이 관계는 분석한 시도 전부에서 같은 방향으로 '
                '확인됐습니다. 우리 리포트의 여섯 고리 가운데 가장 보편적인 '
                '연결입니다.</p>')
    body.append('<p>이 지표가 정하지 못하는 건 <b>시점</b>입니다. 금리가 크게 '
                '흔들리면 순환이 한동안 멈춥니다 — 방향이 맞아도 기다려야 하는 '
                '구간이 생깁니다.</p>')
