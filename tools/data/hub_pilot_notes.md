...
**전제**: '적정'(수요 기준선, `ADV.occupancy.ref[시도]` = `refq`, 기준표 상수)은 이번 재설계에서
...
- **판정 기준이 바뀌었다(커밋 2e8df19)**: 애초 계획한 **CV(변동계수) < 0.3** 기준은 폐기.
  대신 **'안분(share) 없는 존의 치우침'**으로 재정의했다. 이유는 비율 분모·분자의 집계
  단위가 달라서다 — HUB준공은 **존 단위**인데 `refq`는 **시도 단위**라, 안분을 적용하지
  않고 나눈 비율은 스케일 불일치가 아니라 그냥 인구/세대 비중(share)을 재는 꼴이 된다.
  ⚠️ **이 비율 표를 스케일 근거로 읽을 때는 반드시 존 단위끼리(`refq×share`) 또는
  시도 단위끼리 맞춰 비교할 것** — 섞으면 "HUB가 준공을 적게 잡는다"는 오독이 나온다.
- **적정(수요)은 재피팅하지 않았다** — 기준표 상수 유지 원칙 그대로. 대신 **안분 잣대를
  인구 → 주민등록세대수로 전환**(커밋 9c52489)해 존 단위 적정(`zrefq = refq × share`)을
  만든다. 존별 refq 직접 적합은 과적합으로 실패해 폐기(재시도 금지).
- 관련 캘리브레이션 근거는 `memory/agongmap-index-calibration.md` 및
  `tools/zone_floor_cap.py` 참조.
...
>
> `done_q`가 빈 3곳은 **누락이 아니다** — `28115 중구영종출장`·`28116 중구용유출장`·
> `28265 서구검단출장`으로, 실제 물량은 부모 구에 정상 집계된다(서구 28260 준공 98,360세대·
...
> `update-cloud.yml`부터 즉시 구모델(KOSIS 기반)로 복귀. 코드 되돌릴 필요 없음.
> (`hub_derive`가 inactive 시 `done`/`sched`/`units`를 걷어내므로 잔재도 남지 않는다.)

전제: 코드는 완결(Task 1~7, 미러 0.0), 스코어 계산 경로는
`hub_permits.json`의 `meta.activate`로 게이트돼 있다. 아래 절차를
밟기 전까지는 라이브 사이트가 기존(구모델) 스코어를 쓴다. **되돌리기는 언제든
...
- 먼 지평(k13-20) sched≈0 → ~1.8·refq 미상쇄 수요가 refq 비례로 대도시를 부족 쪽으로
  민다. `verify_rankdiff.py`의 `far_demand` 열로 확인 — 대도시가 부당하게 부족이면
  지평 캡 or 문서화 검토(모델 변경은 사용자 판단).
