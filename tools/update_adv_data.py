...
# ---- 건축HUB 시군구 실측 인허가/착공 → 생활권 집계(존별 파생값만 ADV 진입) ------
# LIVEZONE/LZ_GU2SI/LZ_SIDO_FULL은 이 파일 하단(생활권 입주예정 섹션)에 정의돼
# 있지만, 파이썬은 함수 '호출' 시점에만 전역을 참조하므로 위치는 상관없다.
# fetch_livezone() 안의 zone_of/gg_zone과 규칙은 동일하되, 기존 생활권 계산
# 경로(occupancy)를 건드리지 않기 위해 여기서 독립적으로 재구현한다(공유 함수로
# 승격하지 않음 — Task 5 결정).

def load_bdong():
    """code_bdong.json(컬럼-딕셔너리, 46345행)의 활성(말소일자 없음) 행에서
    시군구코드 -> (시도전체명, 시군구명) 표를 구성한다. 10MB라 매 호출마다 통째로
    읽으므로 갱신 배치(주 1회) 경로 밖에서 반복 호출하지 않는다."""
    import math
    ...
    for i in range(n):
        ...
        if not (era is None or (isinstance(era, float) and math.isnan(era))):
            continue   # 말소(폐지)된 행은 제외
        nm = d['시군구명'][k]
        if not isinstance(nm, str) or not nm:
            continue   # 시도 합계행 등 시군구명 결측
        cd = str(d['시군구코드'][k])
        out.setdefault(cd, (d['시도명'][k], nm))
    return out


def _hub_zone_map(bdong):
    """sigunguCd(5) -> zone. fetch_livezone()의 zone_of/gg_zone 규칙을 그대로
    복제한다(승격하지 않고 재구현 — 기존 livezone 경로 영향 없음, Task 5 결정).

    hub_permits.json의 sgg 키는 fetch_hub_permits.fold_groups가 고른 대표(rep)
    코드다. code_bdong.json에서 그 코드의 시군구명은 "성남시"(구 없음) 또는
    "성남시 분당구"(시+구 결합) 두 형태 모두로 나타날 수 있다 — 대표 코드가
    항상 '본체'(구 없는 시 자체) 코드로 뽑힌다고 가정할 수 없다(2026-07-23
    수정: 분당·영통·덕양·기흥 등 LZ_GU2SI에 없는 경기 다구 도시에서 이 가정이
    깨져 gg_zone에 구 이름이 그대로 들어가 존재하지 않는 존을 만들어냈다).
    그래서 시군구명의 선두 토큰(항상 시/군)만 취해 zone_of에 넘긴다 — 아래
    참고.
    """
    ...
    for cd, (sido_full, nm) in bdong.items():
        ...
        if not sd:
            continue
        # code_bdong 시군구명은 항상 "시"(1토큰) 또는 "시 구"(2토큰, 시가 먼저)
        # 형태다. zone_of는 실주소 파싱과 마찬가지로 항상 시/군 레벨 토큰을
        # 받아야 하므로 선두 토큰을 취한다: "성남시 분당구" -> "성남시"
        # (LZ_GU2SI에 없는 경기 다구(多區) 도시라 parts[-1]="분당구"를 쓰면
        # gg_zone('분당구')가 '분당구권'이라는 존재하지 않는 존을 만들어낸다 —
        # 2026-07-23 실측). "창원시 마산합포구" -> "창원시"도 zone_of(('경남',
        # '창원시'))로 바로 해결되고, 구 단독 1토큰("마산합포구")이 들어와도
        # zone_of 내부 LZ_GU2SI가 접어준다.
        parts = nm.split(' ')
        sg = parts[0]
        z = zone_of(sd, sg)
        if z:
...
def _hub_group_reps(bdong):
    """cd(원시 시군구코드) -> 대표(rep) 코드. fetch_hub_permits.fold_groups의
    그룹핑 규칙(시도+이름 선두 토큰으로 묶고, 코드 오름차순 첫 값을 대표로 삼음)을
    순수 함수로 복제한다(TOOLS_DATA를 존중해야 하는 테스트 격리 때문에
    fetch_hub_permits.build_targets()를 직접 재사용하지 않는다 — 그쪽은 항상
    자기 자신의 실제 code_bdong.json 경로를 쓴다).

    hub_permits.json의 sgg 키는 바로 이 rep 코드이므로(구가 나뉜 다구 도시—
    성남·창원·청주·수원·고양·용인 등—는 대표 하나로 접혀 저장된다), Fix B
    완결성 판정 시 개별 구 코드가 아니라 이 rep로 환산해 hp['sgg']/scanned와
    대조해야 한다. 그러지 않으면 대표가 아닌 구 코드는 hp['sgg']에 결코 키로
    나타나지 않아 해당 존이 영원히 미완결(폴백)로 남는 버그가 된다."""
    import collections
...
def hub_derive(adv):
    """hub_permits.json(시군구 실측)을 존별 파생값 두 개로 접어 adv['permits']에 주입.
    원시 시군구 레코드는 절대 adv에 넣지 않는다 — meas/fwd_far만 존별로 남는다.

    Fix A(활성화 게이트): meta.activate가 False/부재면 라이브 지표를 절대
    건드리지 않는다 — meas/fwd_far 키 자체를 쓰지 않고 즉시 반환한다.
    calc()/scCalc()는 이 키들이 없으면 기존 dC·2년 forward로 원자적으로
    폴백하므로, 시딩이 아무리 진행돼도 사용자가 activate를 켜기 전까지
    라이브 화면은 한 글자도 안 바뀐다.

    Fix B(존별 완결성 게이트): activate=True라도, 한 존의 meas/fwd_far는 그
    존을 구성하는 모든 시군구가 '깨끗하게' 수집 완료됐을 때만 emit한다.
    일부만 수집된 존은 여전히 폴백(구 dC·2년)을 쓴다 — 부분 데이터가 조용히
    지표를 흔들지 않게 하기 위함."""
    import collections, datetime
    hp_path = os.path.join(TOOLS_DATA, 'hub_permits.json')
    if not os.path.exists(hp_path):
        print('hub_derive skip: hub_permits.json 없음')
        return
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    import hub_common as H
    hp = json.load(io.open(hp_path, encoding='utf-8'))
    activate = hp.get('meta', {}).get('activate', False)
    if not activate:
        print('hub_derive: inactive (meta.activate=false) — 기존 지표 유지')
        return
    bdong = load_bdong()
    z_of = _hub_zone_map(bdong)
    reps = _hub_group_reps(bdong)
    unresolved_legacy = set(hp.get('meta', {}).get('unresolved_legacy', []))
    scanned = set(hp.get('meta', {}).get('scanned', []))
    sgg_keys = set(hp.get('sgg', {}).keys())
    zone_members = collections.defaultdict(set)   # zone -> 그 존에 속하는 모든 원시 시군구코드
    for cd, z in z_of.items():
        zone_members[z].add(cd)
    complete_zones = set()
    for z, members in zone_members.items():
        remain = members - unresolved_legacy
        if not remain:
            continue   # 전원 unresolved_legacy(예: 부천권) — 상시 폴백, 미완결 취급
        need_reps = {reps.get(c, c) for c in remain}
        if need_reps <= (sgg_keys & scanned):
            complete_zones.add(z)
    WIN_Y = 3                                    # 다년 윈도우(연) — 최근연도 보고 지연 흡수
    now = datetime.date.today()
    ys = [now.year - k for k in range(WIN_Y)]    # 최근 3년(당해 포함)
    meas = collections.defaultdict(int)
    far = collections.defaultdict(lambda: collections.defaultdict(int))
    orphan = 0
    for cd, v in hp.get('sgg', {}).items():
        if cd in unresolved_legacy:
            continue   # 부천 등 옛 구코드 미해결 — Task 6이 dC 폴백 처리
        z = z_of.get(cd)
        if not z:
            orphan += 1
            continue
        for q, n in v.get('permit_q', {}).items():
            if int(q[:4]) in ys:
                meas[z] += n
        for q, n in v.get('start_q', {}).items():
            far[z][H.shift_quarter(q, 13)] += n   # 착공+13분기(~3.25년)=예상 입주
    adv.setdefault('permits', {})
    adv['permits']['meas'] = {z: round(meas[z] / WIN_Y) for z in meas if z in complete_zones}
    adv['permits']['fwd_far'] = {z: dict(d) for z, d in far.items() if z in complete_zones}
    print('hub_derive: zones=%d orphan_sgg_ignored=%d' % (len(meas), orphan))
    print('hub_derive: active, complete_zones=%d incomplete=%d (meas/fwd_far emitted for %d)' % (
        len(complete_zones), len(zone_members) - len(complete_zones), len(complete_zones)))
...
def main():
    ...
    try:
        before = json.dumps((adv.get('permits') or {}).get('meas'), sort_keys=True)
        hub_derive(adv)   # 건축HUB 시군구 실측 → 존별 meas/fwd_far (네트워크 없음, 로컬 파일)
        if json.dumps(adv['permits'].get('meas'), sort_keys=True) != before:
            changed.append('hub_permits(%d)' % len(adv['permits'].get('meas') or {}))
    except Exception as e:
        failed.append('hub_permits'); print('hub_permits skip:', e)
    try:
        h = fetch_holidays()
        if h:
            adv['holidays'] = h
