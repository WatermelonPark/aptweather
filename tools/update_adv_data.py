...
def _hub_zone_map(bdong):
    ...
    코드인데, 실측상 구 분할 도시(성남·창원·청주 등)는 항상 '본체'(구 없는
    바로 그 시 자체) 코드가 그 시 하위 구 코드들보다 수치가 작아 대표로 뽑힌다
    — 즉 code_bdong.json에서 그 코드의 시군구명은 이미 "성남시"처럼 구가 빠진
    형태다. 다만 만일을 대비해 "창원시 의창구"류(시+구 결합) 형태도
    LZ_GU2SI로 구를 접어 안전하게 처리한다.
    """
    ...
    for cd, (sido_full, nm) in bdong.items():
        ...
        if not sd:
            continue
        parts = nm.split(' ')
        sg = parts[-1] if len(parts) > 1 else parts[0]   # "창원시 의창구" -> "의창구", "오산시" -> "오산시"
        z = zone_of(sd, sg)
        if z:
            out[cd] = z
    return out
