...
def build_page(z, calc, stats, pq, others):
    ...
    if row.get('pwarn'):
        ...
        # 등급은 '부족'인데 시장에서는 '역대급 부족'이라 말하는 온도차가
        # 정확히 이 창 차이였다 — 등급은 실측 앵커라 건드리지 않고, 시야는 이 줄이 나른다.
        # 인허가는 같은 해 착공보다 약 1.15배 많은 지표(2012~2025 전국 실측)인데도
        # 적정에 못 미치는 곳만 뜨므로 문구를 세게 써도 과장이 아니다.
        # ⚠️ 여기 적혀 있던 '착공 대비 1.29~1.68배'는 건축HUB 준공예정의 수치였다.
        #    8/11에 인허가 설명으로 잘못 옮겨졌고 2026-09-13에 바로잡았다.
    ...
    h.append('<section><div class="wrap"><h2>어떻게 계산했나</h2>'
             ...
             '인허가는 쓰지 않습니다. 삽을 안 뜬 계획이 섞여 같은 해 착공보다 15%%쯤 많고 '
             '해마다 크게 흔들리기 때문입니다.</p>'
