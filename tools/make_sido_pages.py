...
def build_page(z, calc, stats, pq, others):
    ...
    if row.get('pwarn'):
        ...
        # 등급은 '다소 부족'인데 시장에서는 '역대급 부족'이라 말하는 온도차가
        # 정확히 이 창 차이였다 — 등급은 실측 앵커라 건드리지 않고, 시야는 이 줄이 나른다.
        # 인허가는 부풀려지는 지표(착공 대비 1.29~1.68배)인데도 적정에 못 미치는
        # 곳만 뜨므로 문구를 세게 써도 과장이 아니다.
        h.append('<p class="zwarn">⚠ 3년 너머는 더 얇습니다 — 최근 1년 <b>인허가가 '
                 '적정물량의 %.0f%%</b>에 그쳤습니다. 인허가는 3~4년 뒤 입주로 '
                 '이어지므로, 위 판정이 보는 3년이 지나면 공급은 지금보다 더 '
                 '줄어듭니다.</p>' % (row['pmr'] * 100))
    if row.get('uwarn'):
    ...
    for k, v, note in (
        ...
        ('앞으로 %.0f년 적정물량' % yrs, num(d_need) + '세대',
    ...
    h.append('<section><div class="wrap"><h2>어떻게 계산했나</h2>'
             ...
             # 표에 리터럴 0이 실제로 찍힌다(20장 중 10장·합 35칸, 세종만 16칸).
             # 홈에는 이 뜻을 알려주는 기호 줄이 있는데(index.html 「기호」) 지역
             # 페이지에는 없어서, 같은 표가 결측은 '–'로 쓰면서 0은 맨숫자로 두는
             # 상태였다 — 읽는 사람은 0을 '자료 없음'으로 읽는다(2026-08-15 리뷰).
             # 표 아래 설명 문단은 사용자가 지운 자리라 되살리지 않고, 유지되는
             # 이 절에 한 줄로 넣는다.
             '<p><b>0</b>은 자료가 없다는 뜻이 아니라 <b>그 분기에 실제로 없었다</b>는 '
             '뜻입니다(경계선 위는 준공, 아래는 착공). 값을 못 구한 칸은 <b>–</b>로 둡니다.</p>'
