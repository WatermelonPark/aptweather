...
def running_shortage(done, sched, demol, refq, cur_q, horizon=FUT_HORIZON,
    ...
    # 재고 상하한은 **대칭**이다: ±DEFICIT_CAP*refq(4년치). 2026-08-02 사용자 결정.
    # 예전엔 아래로만 막혀 있어(lo = -CAP) 부족은 4년 상한인데 잉여는 2010년부터
    # 16년치가 쌓였다 — 김포권이 -31,853세대(4년 적정의 5배 남짓)로 찍혀 '과거 4년'
    # 이라는 화면 문구가 한쪽에서만 참이었다. 창을 4년으로 자르는 안(2010년 이전을
    # 통째로 버림)도 재봤지만 등급 28/44곳이 뒤집혀 채택하지 않았다 — 대칭 상한은
    # 만성 부족/과잉 지역이 상한에 붙어 있다는 정보를 유지하면서 표시값만 4년치로
    # 묶는다(등급 변경 1곳: 김포권 -31,853 → -5,057).
    I = 0.0
    cap = DEFICIT_CAP * refq
    for idx in range(ANCHOR, cur_q + 1):
        qk = _qkey(idx)
        I = min(cap, max(-cap, I + done.get(qk, 0) - demol.get(qk, 0) - refq))
    I_now = I
    fut_weighted = 0.0
    demand_sum = 0.0
    supply_weighted = 0.0
...
def build_page(r, allrows, prd, today, punits=None, pidx=None):
    ...
    # 필요하고, 들어올 건 이것뿐".
    # 재고 상하한이 대칭(±4년치)이 된 뒤로 두 방향 모두 '최대 4년치'가 참이다
    # (2026-08-02). 예전엔 아래로만 막혀 있어 '밀린 집'은 4년 상한인데 '쌓인 집'은
    # 16년치가 쌓였고, 그래서 문구도 한쪽만 맞았다.
    b_lab, b_sub = ('그동안 밀린 집', '과거 누적 · 최대 4년치 · 실측') if backlog >= 0 \
              else ('그동안 쌓인 집', '과거 누적 · 최대 4년치 · 실측')
