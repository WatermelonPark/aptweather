...
def start_lead(sts, adv, ps, cur_q, yrs=3):
    ...
    ⚠️ '4년 창 밖을 본다'가 아니다(2026-08-03 사용자 지적으로 정정). 리드타임을
    실측하면 착공→준공 30개월, 인허가→준공 24개월, 인허가→착공 0개월(동시)이다.
    즉 인허가도 착공도 2~2.5년 뒤 준공으로 이어져 **둘 다 16분기 창 안**이고,
    ...
    for _ps in {r['ps'] for r in rows}:
        ...
        if _pct < 50:
            _msg = ('지난 3년 착공이 적정의 <b>%d%%</b>에 그쳤습니다 — 절반도 안 됩니다. '
                    '착공은 약 2년 반 뒤 입주가 되므로, <b>위 일정의 뒷부분</b>은 지금 '
                    '보이는 것보다 더 얇을 수 있습니다.' % round(_pct))
        elif _pct < 95:
            _msg = ('지난 3년 착공이 적정의 <b>%d%%</b>로 미달입니다. 착공은 약 2년 반 뒤 '
        ...
        plead[_ps] = (
            ...
            '<p>%s</p>'
            '<p class="note">%s 착공 실측 %s호 · 같은 기간 적정 %s호. 착공→준공 '
            '리드타임은 실측 약 30개월입니다. 착공은 시도 단위 통계라 생활권별로 나눈 '
            '값이 아닙니다 — 위 일정을 뒷받침하는지 보는 참고 지표입니다.</p>'
