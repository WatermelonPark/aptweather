...
# 독립 시장 공시 대상(⑤ 한계 섹션). 경기 재편(2026-08-05) 후 재측정에서도 서울과
# 음의 잔차 상관이라 공시를 유지한다 — 동남외곽 −0.218(시차를 줄수록 −0.573까지
# 벌어진다), 남서부 −0.271. 옛 이천권·평택권이 각각 여기 들어갔다.
INDEP_ZONES = ('경기남부외곽권',)
...
SEOUL_SYNC = {
    # 경기 8권역 재편(2026-08-05)으로 다시 측정했다. 잔차 상관 0.35 이상만 남긴다 —
    # 예전 개별 시 판정(광명 co·부천 lag 등)은 그룹이 되며 서로 상쇄돼 근거가 사라졌다.
    '경기동남부권': 'co',   # 0.473 @L=0 (성남·용인)
    '경기동부권': 'co',     # 0.444 @L=0 (남양주·구리·하남·광주·양평)
    '인천권': 'lag',        # 기존 판정 유지 — 사용자가 직접 검증한 항목이라 건드리지 않는다
}
...
def running_shortage(done, sched, demol, refq, cur_q, horizon=FUT_HORIZON,
    ...
    for idx in range(cur_q - BACKLOG_WINDOW + 1, cur_q + 1):
        ...
        if idx == cur_q:
            ...
            # 미래 루프는 k=1부터라 sched[cur_q]를 영영 읽지 않으므로 여기서 되돌린다.
            # cur_q는 이 루프에만 등장하므로 산식 안에서의 중복은 없다.
            # ⚠️ 다만 '이중계상 없음'은 **대장(레코드) 단위**에서만 참이다. 수집기는
            # 각 대장을 사용승인일 유무로 done XOR sched에 넣지만, 같은 단지가 동별·
            # 호별 대장으로 쪼개져 일부는 준공·일부는 준공예정으로 남는 경우가 있다.
            # 2026-08-04 실측: sched[2026Q3] 82,500세대 중 5,209세대(6.3%)가 같은
            # (지번, 세대수)로 done 기록도 가진 단지다. 이건 접기(collapse_units_by_
            # project)가 지우도록 설계된 사전 오염인데, 2026-08-03 이전 수집분에는
            # 아직 소급 적용되지 않았다(rebuild_hub_projects가 잘린 부산 4개 구 때문에
            # 중단 상태 — 재시딩 후 실행하면 해소된다). 그때까지 이 항은 최대 6% 과다.
...
def start_lead(sts, adv, ps, cur_q, yrs=3):
    ...
    ⚠️ '4년 창 밖을 본다'가 아니다(2026-08-03 사용자 지적으로 정정). 사이트가 이미
    확립한 리드타임(index.html LEADTIME · /cycle/ 고리④⑤)은 착공→준공 과거 27개월
    → 최근 37개월(r 0.82), 인허가→착공 거의 동시(r 0.74)다. 즉 지난 3년 착공은
    향후 0~37개월 준공이 되어 **16분기(48개월) 창 안**이고, 인허가도 착공과 동시라
    창 밖(4년 이후)을 보여주는 계열은 우리에게 없다.

    ⚠️ 리드타임을 여기서 다시 재려 하지 말 것. 시도 패널 분기 상관으로 재면
    인허가→준공이 24개월로 나와 착공→준공(30개월)보다 짧은, 물리적으로 불가능한
    값이 된다(착공은 인허가 뒤인데 준공까지 더 걸릴 수 없다). 공통 추세·계절성이
    시차 봉우리를 덮는다. 게다가 인허가 계열은 unit이 '호 (연내 누계)'라 1월마다
    리셋되므로 월별처럼 합산하면 그것만으로도 값이 무의미해진다(2026.01에
    346,773 → 13,702). LEADTIME/리포트 값을 단일 출처로 쓴다.

    그럼 왜 쓰나: 착공은 창 안에서 **준공예정(sched)이 아직 못 담은 물량**을 잡는다.
    착공은 했지만 준공예정일이 등록되지 않은 단지가 있고, 그 공백을 메운다.
    아래 편상관이 그 증거다 — 겹침이 낮다는 건 서로 다른 것을 본다는 뜻이다.
    ...
    실측 근거(금리 잔잔 구간, 향후 16분기 물가차감 실질 상승률, n=688):
      ...
      합산                    +0.392
    즉 착공은 준공예정이 못 보는 것을 본다. 다만 **순부족 산식에는 넣지 않는다** —
    시도 단위라 존 해상도가 없고, 넣으면 이중계상(착공→준공예정) 위험이 있다.
    화면에만 교차검증 지표로 세운다.
...
<section><div class="wrap">
  ...
  <p>홈 순위표와 <b>같은 기준</b>(등급으로 묶고, 그 안은 세대수 큰 순)으로 나열합니다.
    홈은 양끝(가장 모자란 곳·가장 남는 곳)만 보여주고, 여기서는 %(n)d곳을 모두 폅니다.<br>
...
<section><div class="wrap">
  <h2>이 숫자는 무엇인가</h2>
  <p><b>순부족</b>은 그 지역에 <b>그동안 모자랐던 양</b>과 <b>앞으로 4년간 필요한 양</b>을 더한 뒤,
    ...
    for _ps in {r['ps'] for r in rows}:
        ...
        if _pct < 50:
            _msg = ('지난 3년 착공이 적정의 <b>%d%%</b>에 그쳤습니다 — 절반도 안 됩니다. '
                    '착공은 약 3년 뒤 입주가 되므로, <b>위 일정의 뒷부분</b>은 지금 '
                    '보이는 것보다 더 얇을 수 있습니다.' % round(_pct))
        elif _pct < 95:
            _msg = ('지난 3년 착공이 적정의 <b>%d%%</b>로 미달입니다. 착공은 약 3년 뒤 '
                    '입주가 되므로, <b>위 일정의 뒷부분</b>도 넉넉하진 않겠습니다.'
                    % round(_pct))
        else:
            _msg = ('지난 3년 착공이 적정의 <b>%d%%</b>입니다. 위 일정의 뒷부분도 물량이 '
                    '받쳐줄 것으로 보입니다.' % round(_pct))
        plead[_ps] = (
            '<div class="lead-box"><b>이 일정이 지켜질까 — 착공으로 교차검증</b>'
            '<p>%s</p>'
            '<p class="note">%s 착공 실측 %s호 · 같은 기간 적정 %s호. 착공에서 '
            '입주까지는 최근 약 3년(과거 2년 남짓에서 늘었습니다). 착공은 시도 단위 '
            '통계라 생활권별로 나눈 값이 아닙니다 — 위 일정을 뒷받침하는지 보는 '
            '참고 지표입니다.</p>'
