...
방법론(기준표): 하단 = 하락→상승 전환(저점) 시점의 입주물량,
...
⚠️ 근본 한계(2026-07 기준):
  ...
  - 따라서 산출값은 참고용이며, 표본·신뢰도를 함께 보고 판단할 것.
    기존 band(ADV.occupancy.band)가 더 안정적일 수 있다.

⚠️ 용어 주의 — 둘은 출처가 다르다:
  - ADV.occupancy.**ref**  = 기준표이 제시한 지역별 적정물량 (사용자 제공, 반올림 상수)
  - ADV.occupancy.**band** = 전환점 실측으로 산출한 [하단, 상단] (커밋 e3fdabd).
    단, 공급발 전환 표본이 없는 지역은 수요 기준선으로 대체하고 좁은 밴드는 ±15% 확폭했다
    → 전 지역이 실측인 것은 아니다.
  calc()는 ref를 우선 쓰고, ref가 없으면 band 중앙을 쓴다.
