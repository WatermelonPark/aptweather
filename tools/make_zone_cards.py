# -*- coding: utf-8 -*-
"""생활권 OG 카드 — share/zone-<존이름>.png (1200x630), 존 하나당 한 장.

카카오톡·SNS에 존 페이지를 공유했을 때 뜨는 미리보기다. 지금까지 45장이
전부 og-brand.png 한 장을 공유해서, 누가 "서울권 공급 분석"을 단톡방에
올려도 미리보기엔 브랜드 로고만 떴다 — 우리 동네 얘기라는 신호가 0이었다.

**숫자를 일부러 넣지 않는다.** 순부족 세대수도, 등급 라벨도 안 넣는다:
  - 카카오톡은 OG 이미지를 한 번 크롤링하면 수일~수주 캐시한다. 숫자를
    박으면 데이터가 갱신된 뒤에도 카톡방엔 옛 숫자가 계속 떠 있고,
    45개 지역을 일일이 캐시 갱신하는 건 비현실적이다.
  - 공급 모델 자체가 개편 중이라(멸실 통합·activate 보류) 숫자와 등급
    라벨이 요동친다. 2026-07-25→08-01 사이 서울권은 30,824→465,831로,
    등급은 '공급 절벽'→'매우 부족'으로 바뀌었다.
그래서 카드에는 안 바뀌는 것만 담는다 — 지역명과 이 서비스가 답하는 질문.
숫자는 링크를 눌러 페이지에서 보면 되고, OG의 역할은 "어느 지역 얘기인지"
알리는 데까지다. 덕분에 주간·월간 갱신 때 재생성할 필요가 없다.
(존이 신설·통폐합될 때만 다시 돌린다. 모델이 안정된 뒤 숫자판을 원하면
 og:image에 ?v=<데이터기준일>을 붙여 캐시를 강제로 깨는 장치가 함께 필요하다.)

존 목록은 zone/ 디렉토리에서 읽는다. calc()를 호출하지 않으므로 공급 모델이
