...
- **적정(수요) 처리 정정**: `ADV.occupancy.ref`는 신쌤이 제공한 **지역별 적정물량 상수**(공급계열에 피팅된 값이 아님, `recompute_bands.py` 문서 확인). 준공물량은 기존 occupancy 공급실적과 **같은 개념**(준공=입주가능)이므로 적정 상수는 **그대로 유지**한다. 필요한 건 재피팅이 아니라 **스케일 검증**: HUB `useInsptDay` 분기 준공 세대가 기존 occupancy 준공실적과 스케일이 맞는지 확인하고, 체계적 차이가 있으면 스케일 계수만 보정. (`estimate_ref_inventory.py`는 원-오프 도구·파이프라인 밖. 입력 스크래치패드는 현존.)
