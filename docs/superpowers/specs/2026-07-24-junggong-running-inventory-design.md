...
- 날짜: 2026-07-24
- 관련 메모리: `hub-permit-integration`, `agongmap-data-pipeline`, `aptweather-project`, `ref-lecture-archive`
...
→ 결론: 건축HUB **단일 소스**로, 단지를 준공(과거)·준공예정(미래)으로 분류하고, 기준표 **러닝재고 모형**에 공급 입력으로 넣어 **누적 순부족**을 산출한다.
...
**목표**: 생활권별 순부족을 건축HUB 단일소스·단지 무중복·러닝재고로 산출. 가격 예측력을 주는 기준표 적정 모형은 유지하되 공급계열을 준공 실측으로 정밀화.
...
- **적정(수요) 처리 정정**: `ADV.occupancy.ref`는 기준표이 제공한 **지역별 적정물량 상수**(공급계열에 피팅된 값이 아님, `recompute_bands.py` 문서 확인). 준공물량은 기존 occupancy 공급실적과 **같은 개념**(준공=입주가능)이므로 적정 상수는 **그대로 유지**한다. 필요한 건 재피팅이 아니라 **스케일 검증**: HUB `useInsptDay` 분기 준공 세대가 기존 occupancy 준공실적과 스케일이 맞는지 확인하고, 체계적 차이가 있으면 스케일 계수만 보정. (`estimate_ref_inventory.py`는 원-오프 도구·파이프라인 밖. 입력 스크래치패드는 현존.)
...
1. **HUB준공 스케일이 occupancy와 다름** — 적정 상수는 유지하되, HUB `useInsptDay` 준공 세대가 기존 occupancy 준공실적과 커버리지·스케일이 다르면(HUB=30세대↑ 공동주택 사업승인) 적정 대비 왜곡. 시드 후 동일 지역·기간 스케일 대조로 계수 보정.
