...
**Architecture:** 수집기 엔진은 재사용하되 버킷을 `done_q`(준공)·`sched_q`(준공예정)로 교체 → `hub_derive`가 생활권별 준공·준공예정 분기시계열을 ADV에 주입 → `calc()`가 러닝재고 순부족을 산출, `scCalc()`가 동치 미러. 적정(수요)은 기준표 상수 유지, 스케일만 검증. activate 게이트로 라이브 무변화 상태에서 구축·시드·검토 후 활성화.
...
- **적정(수요)**: `ADV.occupancy.ref`(기준표 상수) **그대로 유지**. HUB준공 스케일이 occupancy와 다르면 계수만 보정(재피팅 금지).
